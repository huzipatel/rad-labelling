...
import { spreadsheetsApi, exportsApi, tasksApi } from '../services/api'
...
export default function ExportsPage() {
  ...
  const [taskIncludeUnlabelled, setTaskIncludeUnlabelled] = useState(true)
  const [exportMode, setExportMode] = useState<'location-type' | 'task'>('task')
  ...
  const handleExportTaskCSV = async () => {
    ...
    setExporting(true)
    try {
      const response = await exportsApi.exportTaskCsv(selectedTask, taskIncludeUnlabelled)
      ...
      a.download = `task_export_${task?.name.replace(/\s+/g, '_') || selectedTask}_${new Date().toISOString().split('T')[0]}.csv`
      document.body.appendChild(a)
      a.click()
      document.body.removeChild(a)
      window.URL.revokeObjectURL(url)
    } catch (error: any) {
      console.error('Failed to export task CSV:', error)
  ...
  const handleExportCSV = async () => {
    ...
    setExporting(true)
    try {
      const response = await exportsApi.exportLocationTypeCsv(
        selectedType,
  ...
  const handleExportImages = async () => {
    ...
    setExporting(true)
    try {
      const response = await exportsApi.exportImagesZip(
        selectedType,
        selectedCouncil || undefined,
        onlyWithAdvertising
      )
  ...
  return (
    <>
      ...
      {/* Export Mode Toggle */}
      <div className="govuk-radios govuk-radios--inline govuk-!-margin-bottom-6">
        <div className="govuk-radios__item">
          <input
            ...
            name="export-mode"
            type="radio"
            checked={exportMode === 'task'}
            onChange={() => setExportMode('task')}
          />
          <label className="govuk-label govuk-radios__label" htmlFor="export-mode-task">
            Export by Task
          </label>
        </div>
        <div className="govuk-radios__item">
          <input
            className="govuk-radios__input"
            id="export-mode-location"
            name="export-mode"
            type="radio"
            checked={exportMode === 'location-type'}
            onChange={() => setExportMode('location-type')}
          />
          <label className="govuk-label govuk-radios__label" htmlFor="export-mode-location">
            Export by Location Type
          </label>
        </div>
      </div>
      ...
      <div className="govuk-grid-row">
        <div className="govuk-grid-column-two-thirds">
          ...
          {exportMode === 'task' && (
            <>
              <h2 className="govuk-heading-m">Export Task Results</h2>
              <p className="govuk-body">
                Export labelling results for a specific task. This includes all labelled locations
                and optionally unlabelled ones.
              </p>

              <div className="govuk-form-group">
                <label className="govuk-label" htmlFor="taskSelect">
                  Select Task
                </label>
                <select
                ...
                >
                  ...
                  {tasks.map((task) => (
                    <option key={task.id} value={task.id}>
                      {task.name} - {task.council} ({task.completed_locations}/{task.total_locations} labelled)
                    </option>
                  ))}
                </select>
              </div>
                  ...
                  <div className="govuk-checkboxes govuk-checkboxes--small">
                    <div className="govuk-checkboxes__item">
                      <input
                        ...
                        id="taskIncludeUnlabelled"
                        type="checkbox"
                        checked={taskIncludeUnlabelled}
                        onChange={(e) => setTaskIncludeUnlabelled(e.target.checked)}
                      />
                      <label className="govuk-label govuk-checkboxes__label" htmlFor="taskIncludeUnlabelled">
              ...
              <button
                ...
                disabled={!selectedTask || exporting}
              >
                {exporting ? 'Exporting...' : 'Download Task CSV'}
              </button>
            </>
          )}

          {/* Location Type Export */}
          {exportMode === 'location-type' && (
            <>
              <h2 className="govuk-heading-m">Export by Location Type</h2>
              <p className="govuk-body">
                Export all labelling results for a location type, optionally filtered by council.
              </p>
              ...
              {/* CSV Export */}
              <h3 className="govuk-heading-s">Export Labelling Results (CSV)</h3>
              <p className="govuk-body">
              ...
              {/* Image Export */}
              <h3 className="govuk-heading-s">Export Images (ZIP)</h3>
              <p className="govuk-body">
